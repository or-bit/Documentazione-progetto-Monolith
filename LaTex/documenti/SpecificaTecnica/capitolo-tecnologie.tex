...
\begin{itemize}
	\item \textbf{Aspetti positivi}: MondoDB è un database di facile apprendimento, che vanta un'efficienza superiore ai database relazionali non esistendo \textit{join} che porterebbero ad un rallentamento delle operazioni di lettura o scrittura. MongoDB è anche più flessibile di un database \glossario{SQL}, facilitando la rappresentazione su un modello ad oggetti. Altri vantaggi di questa tecnologia sono la scalabilità a seconda delle esigenze dell'applicazione e la libertà dall'uso del linguaggio SQL per la generazione delle query.
